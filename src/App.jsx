...
const updRow   = (cat, i, f, v) => setForm(p => { const u = […p[cat]]; u[i] = { …u[i], [f]: v }; return { …p, [cat]: u }; });
const addRow   = cat => setForm(p => ({ …p, [cat]: […p[cat], { item:””, detail:””, tip:”” }] }));
const delRow   = (cat, i) => setForm(p => ({ …p, [cat]: p[cat].filter((*,idx) => idx !== i) }));
...
const delDayItem = (di, ii) => setForm(p => { const d=[…p.days]; d[di]={…d[di],items:d[di].items.filter((*,idx)=>idx!==ii)}; return {…p,days:d}; });
