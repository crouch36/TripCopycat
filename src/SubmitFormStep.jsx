...
// ── SubmitFormStep ─────────────────────────────────────────────────────────────
// Renders step === "form" inside SubmitTripModal.
// All form state lives in SubmitTripModal and is passed as props.
// Owns only UI-only refs: focalDragging, photoRef, galleryRef.

export default function SubmitFormStep({
  // Form data
  form, setForm, updRow, addRow, delRow, toggleTag,
  // Navigation
  onPhotoSupplement,
  // Cover photo
  ...
  // Submitter
  submitterName, setSubmitterName, submitterEmail, setSubmitterEmail,
  // Submit / footer
  agreedToTerms, setAgreedToTerms, onBack, onSubmit, submitError,
}) {
  // These refs are purely UI concerns — they never trigger re-renders
  ...
  return (
    <>
      ...
      <div style={{ padding:"20px 28px", maxHeight:"65vh", overflowY:"auto", WebkitOverflowScrolling:"touch" }}>
        ...
        {/* Category rows: airfare / hotels / restaurants / bars / activities */}
            ...
            <div style={{ marginBottom:"8px" }}>
              <div style={{ position:"relative", height:"160px", borderRadius:"10px", overflow:"hidden", border:`1px solid ${C.tide}`, cursor:"crosshair", userSelect:"none" }}
                onMouseDown={e => {
                  ...
                  setFocalPoint({ x: Math.round(((e.clientX - rect.left) / rect.width) * 100), y: Math.round(((e.clientY - rect.top) / rect.height) * 100) });
                }}
                onMouseMove={e => {
                  if (!focalDragging.current) return;
                  const rect = e.currentTarget.getBoundingClientRect();
                  setFocalPoint({ x: Math.max(0, Math.min(100, Math.round(((e.clientX - rect.left) / rect.width) * 100))), y: Math.max(0, Math.min(100, Math.round(((e.clientY - rect.top) / rect.height) * 100))) });
                }}
                ...
                onTouchStart={e => {
                  ...
                  const rect = e.currentTarget.getBoundingClientRect();
                  setFocalPoint({ x: Math.round(((touch.clientX - rect.left) / rect.width) * 100), y: Math.round(((touch.clientY - rect.top) / rect.height) * 100) });
                }}
                onTouchMove={e => {
                  e.preventDefault();
                  const touch = e.touches[0];
                  const rect = e.currentTarget.getBoundingClientRect();
                  setFocalPoint({ x: Math.max(0, Math.min(100, Math.round(((touch.clientX - rect.left) / rect.width) * 100))), y: Math.max(0, Math.min(100, Math.round(((touch.clientY - rect.top) / rect.height) * 100))) });
                }}>
